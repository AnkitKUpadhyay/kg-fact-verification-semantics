...
if __name__ == "__main__":
    ...
    # NEW: allow a custom filtered claims pickle
    parser.add_argument("--claims_pickle", type=str, default=None,
                        help="Path to a custom claims pickle (e.g. filtered train). "
                             "If set, this overrides --dataset_type and processes only this file.")
    args = parser.parse_args()

    # ------- helper to load a custom claims pickle into a DataFrame -------
    def df_from_claims_pickle(pickle_path: str) -> pd.DataFrame:
        with open(pickle_path, "rb") as f:
            claims_dict = pickle.load(f)
        rows = []
        for claim, meta in claims_dict.items():
            # Try several common keys for entities; FactKG often has 'Entity_set'
            ent = meta.get("Entity_set") or meta.get("Entities") or meta.get("entities")
            if ent is None:
                ent = []
            rows.append({"Sentence": claim, "Entity_set": ent})
        return pd.DataFrame(rows)

    # ------- choose which data to run on -------
    if args.claims_pickle:
        dataset_types = ["custom"]
    else:
        if args.dataset_type == "all":
            dataset_types = ["train", "val", "test"]
        else:
            dataset_types = [args.dataset_type]  # FIXED typo (was datasety_types)
    ...
    for dataset_type in dataset_types:
        # -------- load DF (either custom pickle or original split) --------
        if dataset_type == "custom":
            df = df_from_claims_pickle(args.claims_pickle)
            suffix = Path(args.claims_pickle).stem  # for filename
        else:
            df = get_df(data_split=dataset_type)
            suffix = dataset_type
        ...
        if args.method == "direct":
            method_name = "direct_filled" if args.fill_if_empty else "direct"
        elif args.method == "one_hop":
            method_name = "one_hop"
        elif args.method == "relevant":
            method_name = "relevant"

        # -------- output file name: include custom suffix when provided ----
        save_filename = f"subgraphs_{method_name}_{suffix}.pkl"
        if SMALL:
            save_filename = "small_" + save_filename
        save_path = save_folder / save_filename
