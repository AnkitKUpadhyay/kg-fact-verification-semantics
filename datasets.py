...
def _env_override_for_split(split: str) -> str | None:
    """
    Optional: allow environment variable override for subgraph file per split.
    FACTKG_SUBGRAPHS_TRAIN_PATH / FACTKG_SUBGRAPHS_VAL_PATH / FACTKG_SUBGRAPHS_TEST_PATH
    """
    key = {
        "train": "FACTKG_SUBGRAPHS_TRAIN_PATH",
        "val": "FACTKG_SUBGRAPHS_VAL_PATH",
        "test": "FACTKG_SUBGRAPHS_TEST_PATH",
    }[split]
    return os.environ.get(key)


def get_subgraphs(data_split, subgraph_type, subgraph_path_override: str | Path | None = None):
    """
    Gets subgraph generated for dataset. Must be first found and saved in `retrieve_subgraph.py`.
    ...
    Args:
        ...
        subgraph_type (str): The type of subgraph to use. Must be either `direct` (only the direct entity
            neighbours), `direct_filled` (the direct entity neigbhours, but if it is empty, replace it with
            all of the entity edges if the entities) or `one_hop` (all of the entity edges).
        subgraph_path_override (str|Path|None): If provided, load this pickle instead of the default.
    ...
    # 1) explicit override param wins
    if subgraph_path_override:
        path = Path(subgraph_path_override)
    else:
        # 2) env var override next
        env_override = _env_override_for_split(data_split)
        if env_override:
            path = Path(env_override)
        else:
            # 3) default location
            filename = f"subgraphs_{subgraph_type}_{data_split}.pkl"
            path = Path(DATA_PATH) / SUBGRAPH_FOLDER / filename
    ...
    if not os.path.isfile(path):
        message = f"Subgraph file for {data_split=} and {subgraph_type=} does not exist, or is in the wrong folder.\n"
        message += f"Tried: {path}\n"
        message += "Create subgraphs (e.g., `python retrieve_subgraphs.py --dataset_type all --method one_hop`)."
        raise FileNotFoundError(message)
...
#     data_split,
#     subgraph_type=None,
#     subgraph_to_use="discovered",
#     model="bert-base-uncased",
#     max_length=512,
#     batch_size=64,
#     shuffle=True,
#     drop_last=True,
#     # NEW: optional per-split override for subgraph pickle paths
#     subgraph_paths: dict | None = None,
# ):
#     """
#     Creates a dataloader for the desired data split and evidence (subgraph).
...
#         subgraph_type (str|None): `direct` | `direct_filled` | `one_hop` | `relevant` | None
#         subgraph_to_use (str): In ["discovered", "connected", "walkable"]
#         model (str): HF model name for tokenizer
#         max_length (int)
#         batch_size (int)
#         shuffle (bool)
#         drop_last (bool)
#         subgraph_paths (dict|None): Optional override mapping per split:
#             {"train": "/abs/path.pkl", "val": "...", "test": "..."}
...
#     if subgraph_type is not None:
#         override_path = None
#         if subgraph_paths and data_split in subgraph_paths and subgraph_paths[data_split]:
#             override_path = subgraph_paths[data_split]
#         subgraphs = get_subgraphs(data_split, subgraph_type, subgraph_path_override=override_path)
...
#             evidence = []
#             for i in range(len(subgraphs)):
#                 # FIX: use connected if NOT empty, else walkable
#                 if subgraphs["walked"][i]["connected"] != []:
#                     evidence.append(subgraphs["walked"][i]["connected"])
#                 else:
#                     evidence.append(subgraphs["walked"][i]["walkable"])
...
# Replace the get_dataloader function in datasets.py with this:
...
def get_dataloader(
    ...
    -- MODIFIED to load LLM-Filtered files if env vars are set --
    """

    # --- START: MODIFICATION FOR LLM-FILTERED FILES ---
    # Check for our new custom env vars
    filtered_key = {
        "train": "FACTKG_FILTERED_TRAIN_PATH",
        "val": "FACTKG_FILTERED_VAL_PATH",
        "test": "FACTKG_FILTERED_TEST_PATH",
    }.get(data_split)
    filtered_path = os.environ.get(filtered_key) if filtered_key else None

    if filtered_path:
        logger.info(f"Loading LLM-Filtered data from: {filtered_path}")
        if not os.path.isfile(filtered_path):
            raise FileNotFoundError(f"Filtered file not found: {filtered_path}")
        
        # Load the single filtered file
        df = pd.read_pickle(filtered_path)
        
        # Adapt columns to what FactKGDataset expects
        df.rename(columns={"claim": "Sentence", "label": "Label"}, inplace=True)
        
        # The 'Label' column in the new file is an int (0 or 1).
        # The original `FactKGDataset` expects a list/tuple like `[0]`.
        # We must wrap it in a list.
        df["Label"] = df["Label"].apply(lambda x: [int(x)])
        
        # Get the evidence from the 'filtered_triples' column
        if "filtered_triples" not in df.columns:
            raise ValueError(f"File {filtered_path} is missing 'filtered_triples' column.")
        
        # Evidence is the list of filtered triples
        evidence = df["filtered_triples"]
    
    else:
        # --- ORIGINAL LOGIC (if no env var is set) ---
        logger.info(f"Loading original data for split: {data_split}")
        df = get_df(data_split)
        if subgraph_type is not None:
            override_path = None
            if subgraph_paths and data_split in subgraph_paths and subgraph_paths[data_split]:
...
def get_graph_dataloader(
    ...
    Args:
        data_split (str): `train`, `val`, or `test`
        subgraph_type (str): `direct` | `direct_filled` | `one_hop` | `relevant`
        online_embeddings (bool)
        bert_model_name (str)
        max_length (int)
        batch_size (int)
        shuffle (bool)
