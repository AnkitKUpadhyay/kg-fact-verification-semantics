...
from improved_qagnn_v2 import ImprovedQAGNN  # <-- STEP 1: ADDED IMPORT
...
def get_args():
    ...
    # --- STEP 2: ADDED ARGUMENTS ---
    parser.add_argument("--use_improved", action="store_true",
                        help="Use improved QA-GNN with cross-attention")
    parser.add_argument("--dual_pooling", action="store_true",
                        help="Use dual pooling (mean+max) for improved QA-GNN")
    # --- END ADDED ARGUMENTS ---
...
if __name__ == "__main__":
    ...
    if args.qa_gnn:
        ...
        # --- STEP 3: MODIFIED MODEL CREATION LOGIC ---
        if args.use_improved:
            logger.info("Using ImprovedQAGNN with Cross-Attention...")
            model = ImprovedQAGNN(
                ...
                use_dual_pooling=args.dual_pooling  # <-- New arg passed
            )
        else:
            logger.info("Using original QAGNN...")
            model = QAGNN(
                args.model_name, n_gnn_layers=args.n_gnn_layers, gnn_hidden_dim=args.gnn_hidden_dim,
                gnn_out_features=args.gnn_out_features, lm_layer_features=args.lm_layer_features,
                gnn_batch_norm=args.gnn_batch_norm, freeze_base_model=args.freeze_base_model,
